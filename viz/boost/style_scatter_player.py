...
def draw_scatter(game_list, config):
    ...
    #     )
    bounds_x = (min(2095, (round(min(bu_data) / 20) * 20) - 10), max(2910, (round(max(bu_data) / 10) * 10) + 10))
    bounds_y = (min(2.5, (round(min(sb_data) * 10) / 10) - 0.1), max(5.1, (round(max(sb_data) * 10) / 10) + 0.1))

    ax_pad = 5 * MARGIN
    tick_px_x, tick_px_y = 350, 250
    tick_jump_x, tick_jump_y = 200, 0.5
    ...
    for name in metrics:
        ...
        if name in ["ApparentlyJack", "Kiileerrz"]:
            utils.draw_scatter_label(draw, name, pos_x, pos_y, radius, 'u')
        elif name in ["virtuoso", "Seikoo"]:
            utils.draw_scatter_label(draw, name, pos_x, pos_y, radius, 'd')
        elif name in ["bananahead", "Chicago", "BeastMode", "Rw9", "Motta", "Sweaty"]:
