    ...
    for region in game_lists:
        ...
        for game in region_list:
            ...
            for shot in game.game_metadata.shot_details:
                ...
                goal_data[against_name]["shots_against"] += 1
                if xG_val >= 0.4:
                    goal_data[against_name]["bc_against"] += 1
...
def draw_scatter(game_lists, config):
    ...
    for_data = [round(np.sum(metrics[name]["shots_against"]) / (metrics[name]["seconds_played"] / 300), 3) for name in metrics]
    bc_data = [round(np.sum(metrics[name]["bc_against"]) / (metrics[name]["seconds_played"] / 300), 3) for name in metrics]
    # for name in sorted(metrics):
    #     print(
    #         "{:<12}".format(name),
    #         metrics[name]['games_played'],
    #         "{:<7}".format(round(np.median(metrics[name]["xG_for"]), 3)),
    #         np.median(metrics[name]["bc_for"])
    #     )
    bounds_x = (min(4, (round(min(for_data) * 10) / 10) - 0.1), max(12, 0.1 + (round(max(for_data) * 10) / 10)))
    bounds_y = (min(0.75, (round(min(bc_data) * 10) / 10) - 0.1), max(3.1, 0.1 + (round(max(bc_data) * 10) / 10)))
    #print(bounds_x, bounds_y)

    ax_pad = 5 * MARGIN
    tick_px_x, tick_px_y = 450, 250
    ...
    draw.text((ax_pad + (plot_width / 2), get_y(ax_pad - (1.75 * MARGIN), height)), 
        "Shots Against per 5:00", fill=DARK_GREY, font=constants.BOUR_60, anchor='ma')
    ...
    # Y-axis
    y_label_len = round(draw.textlength("Big Chances Against per 5:00", constants.BOUR_60))
    y_label_img = Image.new("RGB", size=(y_label_len, 60), color=WHITE)
    y_label_draw = ImageDraw.Draw(y_label_img)
    y_label_draw.text((0, 0), "Big Chances Against per 5:00", fill=DARK_GREY, font=constants.BOUR_60)
    ...
    draw.multiline_text((for_pos - 15, MARGIN), "Median Shots\nAgainst", 
    ...
    for name in metrics:
        radius = 15
        val_x = np.sum(metrics[name]["shots_against"]) / (metrics[name]["seconds_played"] / 300)
        val_y = np.sum(metrics[name]["bc_against"]) / (metrics[name]["seconds_played"] / 300)
        ...
        elif name in ["TEAM VITALITY"]:
            utils.draw_scatter_label(draw, name, pos_x, pos_y, radius, "d")
        elif name in ["GEN.G MOBIL1", "OG ESPORTS", "SPACESTATION"]:
...
def create_image(game_lists, config):
    ...
    img.save(os.path.join(config["img_path"], "team_def_xG.png"))
