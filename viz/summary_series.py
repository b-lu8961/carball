...
def draw_field(team_name, game_list):
    ...
    team, color_set, player_names = None, None, []
    ...
    for game in game_list:
        ...
        if team is None:
            ...
            team = [team for team in game.teams if team.is_orange == team_name][0]
    ...
    return img, (goals, xG, shots, saves, demos, int(boost)), color_set, sorted(player_names, key=str.casefold)

def create_image(team_one, team_two, team_keys, wins, game_list, config):
    img_width, img_height = (2 * round(constants.MAP_X)) + 1200 + (MARGIN * 4), round(constants.MAP_Y) + 550 + (MARGIN * 2)
    ...
    # Main field images
    blue_img, blue_counts, blue_colors, blue_players = draw_field(team_one, game_list)
    ...
    orange_img, orange_counts, orange_colors, orange_players = draw_field(team_two, game_list)
    ...
    # Team info
    t1_pos = (int(img_width / 2) - 475, field_top - 150)
    t1t_pos = (int(img_width / 2) - 120, field_top - 150)
    draw.text((blue_left + (blue_img.height / 2), t1_pos[1]), team_keys[0], 
            fill=constants.TEAM_INFO["RL ESPORTS"]["c1"], font=constants.BOUR_100, anchor="ma")
    draw.text((blue_left + (blue_img.height / 2), t1_pos[1] + 110), " | ".join(blue_players), 
            fill=LIGHT_GREY, font=constants.BOUR_60, anchor="ma")
    _, _ = utils.draw_team_logo(img, MARGIN, file_name=constants.TEAM_INFO[team_keys[0]]["logo"], pos=(t1_pos[0], t1_pos[1] - 70))
    draw.text(t1t_pos, str(wins[0]), font=constants.BOUR_100, fill=constants.TEAM_INFO[team_keys[0]]["c1"], anchor="ma")
    draw.rounded_rectangle([
        (t1_pos[0] + 375 - 80, t1_pos[1] + 50 - 60), (t1_pos[0] + 375 + 35, t1_pos[1] + 50 + 65)
    ], 20, outline=constants.TEAM_INFO[team_keys[0]]["c2"], width=5)

    t2_pos = (int(img_width / 2) + 275, field_top - 150)
    t2t_pos = (int(img_width / 2) + 123, field_top - 150)
    draw.text((orange_left + (blue_img.height / 2), t2_pos[1]), team_keys[1], 
            fill=constants.TEAM_INFO["RL ESPORTS"]["c2"], font=constants.BOUR_100, anchor="ma")
    draw.text((orange_left + (blue_img.height / 2), t2_pos[1] + 110), " | ".join(orange_players), 
            fill=LIGHT_GREY, font=constants.BOUR_60, anchor="ma")
    _, _ = utils.draw_team_logo(img, MARGIN, file_name=constants.TEAM_INFO[team_keys[1]]["logo"], pos=(t2_pos[0], t2_pos[1] - 60))
    draw.text(t2t_pos, str(wins[1]), font=constants.BOUR_100, fill=constants.TEAM_INFO[team_keys[1]]["c2"], anchor="ma")
    draw.rounded_rectangle([
        (t2_pos[0] - 175 - 35, t2_pos[1] + 50 - 60), (t2_pos[0] - 175 + 80, t2_pos[1] + 50 + 65)
    ], 20, outline=constants.TEAM_INFO[team_keys[1]]["c1"], width=5)
    ...
    for i in range(len(labels)):
        ...
        if i == 0:
            draw.ellipse([
                (blue_x - 60, field_top + y_pad - 30), (blue_x + 60, field_top + y_pad + 90)
            ], fill=blue_colors[0], outline=blue_colors[1], width=3)
            draw.ellipse([
                (orange_x - 60, field_top + y_pad - 30), (orange_x + 60, field_top + y_pad + 90)
        ...
        draw.rectangle([
            (img_mid - 275, field_top + y_pad - 20), (img_mid - 275 + split, field_top + y_pad + 80)
        ], fill=constants.TEAM_INFO["RL ESPORTS"]["c1"])
        draw.rectangle([
            (img_mid - 275 + split, field_top + y_pad - 20), (img_mid + 275, field_top + y_pad + 80)
        ...
        draw.text((img_mid, field_top + y_pad), labels[i], fill=BLACK, stroke_fill=WHITE, stroke_width=3, font=constants.BOUR_60, anchor="ma")
        draw.text((blue_x, field_top + y_pad), blue_text, fill=BLACK, font=constants.BOUR_60, anchor="ma")
        draw.text((orange_x, field_top + y_pad), orange_text, fill=BLACK, font=constants.BOUR_60, anchor="ma")

    # # Legend below detail text
    utils.draw_circle_legend(draw, field_top + y_pad + 75, MARGIN, img_mid + 430, MARKER_SIZE, constants.BOUR_40, 
    ...
    img.save(os.path.join(config["img_path"], "summary.png"))

def main():
    team_one, team_two = False, True
    team_keys = ["OG", "REBELLION"]
    wins = [4, 3]
    key = "RLCS"
    base_path = os.path.join("RLCS 24", "Major 2", "[1] Major", "Tiebreaker", "OG vs SR")
    game_list = utils.read_series_data(os.path.join("replays", base_path))
