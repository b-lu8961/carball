...
def calculate_stats(tag):
        ...
        for region in reg_list:
            ...
            for game in game_list:
                ...
                for goal in game.game_metadata.goals:
                    ...
                    pn = utils.get_player_label(goal.assister, reg)
    ...
    print(len(impact_assists))
    stat_data = dict(sorted(impact_assists.items(), key=lambda item: (-item[1], str.casefold(item[0]))))
    old_ranks = dict(sorted(impact_assists.items(), key=lambda item: (-item[1] + event_data[item[0]], str.casefold(item[0]))))
...
def main():
    ...
    config = {
        ...
        "img_name": os.path.join("RLCS 24", "Leaderboards", "Season", folder, "impact_assists.png")
    }
