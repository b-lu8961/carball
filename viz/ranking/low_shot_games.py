        ...
        for region in reg_list:
            ...
            for game in game_list:
                ...
                for team in game.teams:
                    ...
                    teams[team.is_orange] = tn
                    game_shots_faced[tn] = 0
                    if tn not in low_shot_games: 
                        low_shot_games[tn] = 0
                        event_data[tn] = 0
                        team_info[tn] = reg

                for player in game.players:
                    game_shots_faced[teams[not player.is_orange]] += player.shots
                
                if game_shots_faced[teams[True]] < 5 and game.game_metadata.last_second <= 1:
                    low_shot_games[teams[True]] += 1
                    if tag in game.game_metadata.tag or ("Major 2" in game.game_metadata.tag and "Open Qualifiers 1" in game.game_metadata.tag):
                        event_data[teams[True]] += 1
                if game_shots_faced[teams[False]] < 5 and game.game_metadata.last_second <= 1:
                    low_shot_games[teams[False]] += 1
                    if tag in game.game_metadata.tag or ("Major 2" in game.game_metadata.tag and "Open Qualifiers 1" in game.game_metadata.tag):
                        event_data[teams[False]] += 1

            print(region)
        
    print(len(low_shot_games))
    stat_data = dict(sorted(low_shot_games.items(), key=lambda item: (-item[1], str.casefold(item[0]))))
    old_ranks = dict(sorted(low_shot_games.items(), key=lambda item: (-item[1] + event_data[item[0]], str.casefold(item[0]))))
...
def main():
    ...
    config = {
        ...
        "img_name": os.path.join("RLCS 24", "Leaderboards", "Season", folder, "low_shot_games.png")
    }
