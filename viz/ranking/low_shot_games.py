    ...
    for region in regions:
        ...
        for maj_name in ["Major 1"]:
            ...
            for game in game_list:
                ...
                if game_shots_faced[teams[True]] < 5 and game.game_metadata.last_second <= 0:
                    low_shot_games[teams[True]] += 1
                if game_shots_faced[teams[False]] < 5 and game.game_metadata.last_second <= 0:
...
def create_image(game_list, config):
    team_data, team_info = calculate_stats(game_list)
    NUM_SPOTS = 15
    ROW_Y = 125
...
def main():
    ...
    config = {
        ...
        "img_name": os.path.join("RLCS 24", "Leaderboards", "Season", f"Post OQ{reg_num}", "low_shot_games.png")
    }
