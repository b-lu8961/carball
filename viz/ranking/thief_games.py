...
def calculate_stats(tag):
        ...
        for region in reg_list:
            ...
            for game in game_list:
                for player in game.players:
                    ...
                    if player_steals >= 7 and game.game_metadata.last_second <= 1:
                        thief_games[pn] += 1
    ...
    old_ranks = dict(sorted(thief_games.items(), key=lambda item: (-item[1] + event_data[item[0]], str.casefold(item[0]))))
...
def main():
    ...
    config = {
        ...
        "img_name": os.path.join("RLCS 24", "Leaderboards", "Season", folder, "thief_games.png")
    }
