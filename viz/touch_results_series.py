...
def create_image(team_one, team_two, game_list, config):
    img = Image.new(mode = "RGBA", size = (IMAGE_X, IMAGE_Y), color = WHITE)
    draw = ImageDraw.Draw(img)

    # Team info
    draw.text(((IMAGE_X / 2 - 400), 300), team_one, fill=constants.TEAM_INFO["RL ESPORTS"]["c1"], font=constants.BOUR_80, anchor="ma")
    draw.text(((IMAGE_X / 2 + 400), 300), team_two, fill=constants.TEAM_INFO["RL ESPORTS"]["c2"], font=constants.BOUR_80, anchor="ma")
    ...
    pad = 520
    for key in sorted(touch_results):
